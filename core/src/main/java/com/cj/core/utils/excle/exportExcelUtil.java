package com.cj.core.utils.excle;

import com.cj.core.domain.ExpotrExcle;
...
//                }
    ...
    public static void exportExcel3(
            String[] sheetNames,
            List<List<List<ExpotrExcle>>> excless,
            List<List<List<ExpotrExcle>>> excleDatass,
            HttpServletResponse response,
            String fileName
    ) throws Exception {
        ...
        for (int x = 0; x <sheetNames.length ; x++) {
            ...
            // 声明一个sheeet
            HSSFSheet sheet = wb.createSheet();
            wb.setSheetName(x, sheetNames[x]);
            //设置列宽度大小
            sheet.setDefaultColumnWidth((short) 15*1);
            //设置行高
            sheet.setDefaultRowHeight((short)(25*20));
            ...
            List<List<ExpotrExcle>> excles = excless.get(x);
            //生成表头
            for (int m = 0; m <excles.size() ; m++) {
                //生成第n行
                HSSFRow row = sheet.createRow(m);
                //第n行
                List<ExpotrExcle> expotrExcles = excles.get(m);
                //第n行动态合并单元格
                for (int n = 0; n < expotrExcles.size(); n++) {
                    ExpotrExcle expotrExcle = expotrExcles.get(n);


                    if (expotrExcle.getFirstRow() !=expotrExcle.getLastRow() || expotrExcle.getFirstCol() != expotrExcle.getLastCol()){
                        //设置合并单元格
                        CellRangeAddress cellRangeAddress = new CellRangeAddress(expotrExcle.getFirstRow(), expotrExcle.getLastRow(), expotrExcle.getFirstCol(), expotrExcle.getLastCol());
                    ...
                    cell.setCellValue(expotrExcle.getMsg());
            ...
            List<List<ExpotrExcle>> excleDatas = excleDatass.get(x);
            if (excleDatas.size()>0){



                //生成数据
                for (int m = 0; m <excleDatas.size() ; m++) {
                    //生成第n+i行
                    HSSFRow row = sheet.createRow(excles.size()+m);
                    List<ExpotrExcle> expotrExcles = excleDatas.get(m);
    ...
    public void testPOI(){
        ...
        try {
            ...
            path="D:/file/oil/测试Eexle导出.xlsx";
//1、输出的文件地址及名称
